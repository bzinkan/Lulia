...
**Current status**: Phases 1-29 complete locally. All core services run via Docker Compose. Pre-production: ready for AWS deployment hardening and beta testing, with JWT tenant isolation now enforced across protected teacher routes and the first prebuilt activity library slice in place.
...
- `docs/PREBUILT_ACTIVITIES_DISCOVERY.md` - Current interactive flow and prebuilt activity storage decision
...
25. Prebuilt interactive activities use Git-backed JSON as the source of truth and seed into the `prebuilt_activities` table for runtime browsing/filtering. Teacher use/copy creates normal `assignments` + `interactive_activities` rows from the canonical record; never mutate the canonical prebuilt record for teacher customization. Keep reusable renderers additive and preserve the existing generated interactive flow.
...
| 29 | Prebuilt interactive activity library MVP slice: added canonical `prebuilt_activities` table, Git-backed JSON source under `data/prebuilt_activities/`, Pydantic validation, idempotent seed script with `--dry-run`/`--course`/`--status`, and discovery note at `docs/PREBUILT_ACTIVITIES_DISCOVERY.md`. Added `/api/v1/prebuilt-activities` browse/get/course-map/preview/use endpoints; use/copy deploys standalone HTML into the existing `assignments` + `interactive_activities` flow. Added `visual_study` renderer plus document/model starter renderers, five seed activities across Science, Algebra I, English 9, and U.S. History, `/prebuilt` teacher browser/preview/use UI, Docker data mount/image include for prebuilt JSON, and prebuilt smoke tests. |
...
docker compose exec api python scripts/seed_prebuilt_activities.py --dry-run
docker compose exec api python scripts/seed_prebuilt_activities.py --status published
docker compose exec api pytest tests/ -v                  # Run tests (40/40 passing)
```
