...
async def generate_slides(req: GenerateSlidesRequest):
    ...
    try:
        ...
        if not result:
            return {"error": "Failed to create Google Slides. Is your Google account connected?"}

        return {
            "slides_url": result.get("presentation_url", result.get("url", "")),
