...
import { Settings, Link2, CheckCircle, XCircle, ExternalLink, Calendar } from 'lucide-react';
...
function SettingsPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && window.location.search.includes('google=connected')) setJustConnected(true);
    checkStatus();
  }, []);

  async function checkStatus() {
    try {
      const data = await apiFetch('/api/v1/classroom/status');
